...
local full_completed_cmd_tbl = string.format(
	"%s %s %s %s completed",
	task,
	column_config,
	column_label_config,
...
-- For json output
local full_completed_cmd_json = string.format(
	"%s %s %s %s export",
	task,
	column_config,
	column_label_config,
...
function M.check_task_executable()
	if vim.fn.executable(task) ~= 1 then
		error(string.format("Could not find executable %s. Cannot report on tasks.", task))
	end
end
...
function M.report_completed_tasks_tbl()
	...
	if vim.v.shell_error ~= 0 then
		return "Report failed or none completed within 48 hours."
	end
